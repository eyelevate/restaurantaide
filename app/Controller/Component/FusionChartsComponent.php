...
class FusionChartsComponent extends Component
{
	...
	{
		parent::beforeFilter();
		//send the cookie to the users/login view
		$this->set('company_info',$this->Cookie->read('Restaurant'));
		//check to see if the cookie still exists otherwise logout and send to companies controller
		if ($this->Cookie->read('Restaurant.company_id')=='') {
			$this->Session->setFlash('Your company session has ended. Please log back into your company.');
			return $this->redirect(array('controller'=>'companies','action'=>'login'));
			$this->Auth->logout();
		} else {
			$this->set('logged_user',AuthComponent::user('username'));
		}
		$this->Auth->allow('*');
