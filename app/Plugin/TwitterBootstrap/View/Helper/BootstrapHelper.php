...
class BootstrapHelper extends AppHelper {
	...
	public function flashes($options = array()) {
		...
		if (isset($options["closable"]) && $options["closable"]) {
			$options["keys"][] = '<a class="close" data-dismiss="alert">&times;</a>';
		}		
		$keys = $options["keys"];
		unset($options["keys"]);
		$out = '';
	...
	public function block($message = null, $options = array()) {
		...
		//adding additional class to the options
		if (isset($options["addClass"]) && $options["addClass"]){
			$class = "alert alert-block{$style} ".$options["addClass"];
		} else {
